...
def parse_args():
    ...
    parser.add_argument('--inference_id', help='inference_id', type=int, default=0)
    parser.add_argument('--root', help='root location', default='')
    parser.add_argument('--json_out', help='output result file', default='')
    # only for TSN3D
...
def main():
    ...
    if args.out and rank == 0:
        ...
        pred_label = [np.argmax(x) for x in results]

        # Here add result into json
        # 1. load inference result from disk, as we need to extract
        with open(os.path.join(args.root, "inference_list_{}.txt".format(args.inference_id)), 'r') as file_reader:
            raw_data = []
            for idx, lines in enumerate(file_reader):
                raw_data.append(lines.replace("\n", "").split(" ")[0].split("/")[1])

        print("Check order of raw_data: ", raw_data)
        for idx, file in enumerate(raw_data):
            json_path = os.path.join(args.json_out, file+".json")
            if not os.path.exists(json_path):
                print("Not able to locate json path : {}".format(json_path))
            with open(json_path) as json_file:
                data = json.load(json_file)
                data["prob"] = str(max(results[idx]))
                if pred_label[idx]:
                    data["result"] = "False"
                else:
                    data["result"] = "True"
            save_metadata(data, json_path)
