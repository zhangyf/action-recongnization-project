...
def main():
    ...
    if args.out and rank == 0:
        ...
        with open(os.path.join("data", "ucf101","inference_result.txt"),'r') as file_reader:
            raw_data = []
            for idx, lines in enumerate(file_reader):
                raw_data.append(" ".join(lines.replace("\n", "").split(" ")[:-1]+[str(pred_label[idx])])+"\n")

        with open(os.path.join("data", "ucf101", "inference_result.txt"), "w") as file_writer:
