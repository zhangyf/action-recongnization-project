...
def extract_frame_from_movie(source_file, target_folder):
    # target_folder = "original/throw_up_down_0"
    # source_file = ["source_data/video/throw/throw_up_down_0.mp4"]
    if not os.path.exists(os.path.join(".", target_folder)):
        os.makedirs(os.path.join(".", target_folder), exist_ok=False)
    else:
        # Assume that we have generate video
        return
    for video_name in source_file:
        v_name, v_affix = video_name.split(".")
        ...
        while success:
            filename = v_name + "_" + str(idx) + ".jpg"
            idx += 1
            print("Save to: {}".format(os.path.join(os.path.join(".", target_folder), filename)))
            status = cv2.imwrite(os.path.join(os.path.join(".", target_folder), filename), frame)
            if not status:
...
def prepare_movie_with_timestamp(source_folder, target_folder, action_name, clip_len=64):
    ...
    if not os.path.exists(os.path.join(".", target_folder, "videos")):
...
def generate_annotation(target_folder, class_idx, video_list, action_name, seed=42, test_ratio=0.2):
    """
    ...
    with open(os.path.join(target_folder, "annotations", "trainlist{:02d}.txt".format(1)), 'w') as file_writer:
        for path in X_trainval:
            file_writer.writelines(path + " " + str(class_idx[action_name]) + "\n")
    with open(os.path.join(target_folder, "annotations", "testlist{:02d}.txt".format(1)), 'w') as file_writer:
        for path in X_test:
            file_writer.writelines(path + " " + str(class_idx[action_name]) + "\n")
...
if __name__ == "__main__":
    ...
    for action in action_name:
        ...
        generate_annotation(target_folder, class_idx, video_list, action, seed=seed, test_ratio=test_ratio)
