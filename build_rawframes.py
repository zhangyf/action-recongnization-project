...
def parse_args():
    ...
    parser.add_argument("--online_inference", action='store_true', default=False,
                        help='Whether you conduct online inference or not')
    parser.add_argument("--inference_id", type=int, default=0, help='inference file id')
...
if __name__ == '__main__':
    ...
    if args.level == 2:
        ## online inference is only available to level 2 dataset
        if args.online_inference:
            fullpath_list = []
            with open(os.path.join(args.src_dir.replace("inference_video", ""), "inference_list_{}.txt".format \
                (args.inference_id))) as file_loader:
                for path in file_loader:
                    fullpath_list.append(os.path.join(args.src_dir, path.split(" ")[0]+"."+args.ext))
        else:
